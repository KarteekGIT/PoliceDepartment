...
header('Location: pt_data.php?action=list');
}
if($action=='move_pt'){
$id=$_GET['id'];
...
?>
	...
	$year=date('Y');
	if($action=='addnew'){
	$in = "INSERT INTO police_cases(subd,circle, ps_name, crime_no,section_law,present_stage,reasons,crime_head,user_id,year) VALUES('$subd','$circle','$ps_name','$crime_no','$section_law','$present_stage','$reasons','$crime_head','$usid','$year')";
            ...
            <fieldset>
                <div class="widget first">
                    <div class="head"><h5 class="iList">Add New Case</h5></div>
                        ...
                        <div class="rowElem"><label>Present Stage:</label><div class="formRight"><input type="text" name="present_stage" class="validate[required]" value="<?php if($action=='edit'){echo $edi['present_stage'];}?>"/></div><div class="fix"></div></div>
                        
                        <div class="rowElem">
                        <label>Reasons :</label>
                        <div class="formRight">
                        <select name="reasons"  class="validate[required]" id="req3">
                    ...
                    <div class="rowElem"><label>Crime Head:</label><div class="formRight"><input type="text" name="crime_head" class="validate[required]" value="<?php if($action=='edit'){echo $edi['crime_head'];}?>"/></div><div class="fix"></div></div>
...
if($_SESSION['police']['type']=='Admin'){
			$sel="select * from police_cases where pt='Yes'";
}else{
	$sel="select * from police_cases where subd='$subd' and circle='$circle' and ps_name='$ps_name' and pt='Yes'";
	}
                    ...
                    <tr <?Php if($sno%2==0){?>class="gradeX"<?Php }else{?>class="gradeC"<?php }?>>
                       <td><?php echo $sno;?></td>
									...
									<td>
										 ...
										 <a href="pt_data.php?action=edit&id=<?php echo $rec['id'];?>" title="Edit"><img src="images/icons/middlenav/pencil.png" width="18" alt="Edit" /></a> &nbsp;&nbsp;
										 <a href="pt_data.php?action=delete&id=<?php echo $rec['id'];?>" onClick="return confirm('Are you sure you want to delete ?')" title="Delete"><img width="18" src="images/icons/middlenav/denied.png" alt="Delete" /></a>
                                         &nbsp;&nbsp;
									</td>
                                    <?php }?>
                    </tr>
